...
import com.pingjin.date.TimeUtil;
...
public class ExcelReadUtil {
	...
	public static List<Object> excelRead(File file, Integer startIndex, String[] properties, Class obj) throws Exception{
		Workbook book = null;
		try {
		...
		return getExcelContent(book,startIndex,properties,obj);    
	}
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public static List<Object> excelRead(String filePath,Integer startIndex,String[] properties,Class obj) throws Exception{
		File file = new File(filePath);
		if(!file.exists()){
			throw new Exception("指定的文件不存在");
		}
		return excelRead(file, startIndex, properties, obj);
	}
	 ...
	 */
	@SuppressWarnings("rawtypes")
	private static List<Object> getExcelContent(Workbook book, Integer startIndex, String[] properties,
		...
		for(int numSheet = 0 ; numSheet < book.getNumberOfSheets(); numSheet++){
			...
			for(int numRow = startIndex ; numRow <= sheet.getLastRowNum() ; numRow++){   //一个row就相当于一个Object
		...
		if ("java.lang.String".equals(type) || "String".equals(type)) {
			...
			if (value.length() > 0){//当长度为19(yyyy-MM-dd HH24:mm:ss)或者为14(yyyyMMddHH24mmss)时Date格式转换为yyyyMMddHH24mmss
				if(value.length() == 19 || value.length() == 14){  
					oo[0] = TimeUtil.getDate(value, "yyyy-MM-dd HH:mm:ss");
				}
				else{     //其余全部转换为yyyyMMdd格式
					oo[0] = TimeUtil.getDate(value, "yyyy-MM-dd");
				}
			}
	...
	private static String getValue(Cell cell) {  
        if (cell.getCellType() == cell.CELL_TYPE_BOOLEAN) {  
			...
			if (DateUtil.isCellDateFormatted(cell)) {
				//如果是日期 则日期转换
				return TimeUtil.getTimeStr(cell.getDateCellValue(), "yyyy-MM-dd");
			} else {
	...
	public static void main(String[] args) {
		File file = new File("D:/84e0fea49776437bad1f01ffd470bcb2.xlsx");
		String[] pro = new String[] {"entid","merName","merCode","ceditCode","campanyType","category","categoryName","industryCode","industryCodeName","permitBusiScope",
				"generalBusiScope","busiBeginTime","busiEndTime","post","relaPhone","email","foundDate","auditDate","registrationAuthority","state","registeredCapital",
				"registeredCapitalKind","busiScope","busiWay","busiScopeWay","otherBusiScope","address","residenceCode","busiPlace","busiPlaceCode","capitalCollection",
				"owner","logoutDate","revocationDate","annualInspectionDate","peopleNumber","propertyRight","incorporationWay","partnerNumber","limitedPartnership",
				"partnershipMode","employeeNumber"};
		try {
			//导入
			/*List<?> list =  excelRead(file, 1, pro, MerMerchantReal.class);
			System.out.println(list.size());
			List<MerMerchantReal> data = (List<MerMerchantReal>) list;
			for (MerMerchantReal msg : data) {
				Gson gson = new Gson();
